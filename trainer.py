...
from dataset_Walnut import Projections_Dataset_Walnut
...
def main(cfg : DictConfig):
    params = dict(cfg)
    train_dataset = Projections_Dataset(**params)
    validation_dataset = Projections_Dataset_Walnut(**params)
    ...
    geometry = train_dataset.geom[0]
    print("Parameter loaded")
    model = Pipeline(geometry=geometry,learning_rate=params['base_lr'], num_data=train_dataset.__len__(), num_epoch=params['num_epochs'])
    # model = model.load_from_checkpoint("E:\MasterArbeit\code\Defrise-and-Clack-reconstruction\checkpoints\checkpoint-v1.ckpt", geometry=geometry, learning_rate=params['base_lr'], num_data=train_dataset.__len__(), num_epoch=params['num_epochs'])
    ...
    wandb_logger = WandbLogger(project="Chengze_DandC_circular", name=f"{params['num_epochs']}-num_epochs_lr{params['base_lr']}")  # TODO some logging routines might have to be adjusted
